...
@section('content-title', 'Editar Categoría Histórica')
    ...
    <div class="col-md-12">
      <div class="card card-info">
        ...
        <div class="card-header">
          <h3 class="card-title">Editar Categoría: {{ $historicalCategory->name }}</h3>
        </div>
        <form class="form-horizontal" method="POST" action="{{ route('admin.historical-categories.update', $historicalCategory) }}"
          ...
          <div class="card-body">
            ...
            <div class="form-group row">
              <div class="col-md-6 col-sm-12">
                ...
                <input id="name" class="form-control @error('name') is-invalid @enderror" name="name"
                  value="{{ old('name', $historicalCategory->name) }}" required autofocus
              ...
              <div class="col-md-3 col-sm-12">
                ...
                <input id="sort_order" class="form-control @error('sort_order') is-invalid @enderror" name="sort_order"
                  type="number" min="0" value="{{ old('sort_order', $historicalCategory->sort_order) }}"
              ...
              <div class="col-md-3 col-sm-12">
                ...
                <div class="icheck-success">
                  <input type="checkbox" name="is_active" id="is_active" value="1"
                    {{ old('is_active', $historicalCategory->is_active) ? 'checked' : '' }}>
                  <label for="is_active">
                    Categoría activa
                  </label>
                </div>
            ...
            <div class="form-group row">
              <div class="col-sm-12">
                ...
                <textarea id="description" class="form-control @error('description') is-invalid @enderror" name="description"
                  rows="3" placeholder="Descripción opcional de la categoría">{{ old('description', $historicalCategory->description) }}</textarea>
              ...
              <div class="col-sm-12">
                ...
                <div>
                  <span class="category-preview" id="category-preview">
                    <i class="{{ old('icon', $historicalCategory->icon ?? 'fas fa-folder') }}" id="preview-icon"></i>
                    <span id="preview-name">{{ old('name', $historicalCategory->name) }}</span>
                  </span>
                </div>
              ...
              <div class="col-sm-12">
                ...
                @if ($historicalCategory->background_image)
                  <div class="mt-2">
                    ...
                    <br>
                    <img src="{{ asset($historicalCategory->background_image) }}" alt="Imagen actual" class="img-thumbnail" style="max-width: 200px; max-height: 150px; object-fit: cover;">
                    <br>
                    <small class="text-muted">{{ $historicalCategory->background_image }}</small>
                  </div>
                @endif
            ...
            <div class="form-group row">
              <div class="col-sm-12">
                ...
                <input type="hidden" id="icon" name="icon" value="{{ old('icon', $historicalCategory->icon ?? 'fas fa-folder') }}">
                ...
                <div class="icon-grid" id="icon-grid">
                  @php
                    ...
                    $currentIcon = old('icon', $historicalCategory->icon ?? 'fas fa-folder');
                  @endphp

                  @foreach($icons as $iconClass => $iconLabel)
                    <div class="icon-option {{ $currentIcon == $iconClass ? 'selected' : '' }}"
                         data-icon="{{ $iconClass }}"
                         title="{{ $iconLabel }}">
                      <i class="{{ $iconClass }}"></i>
                    </div>
